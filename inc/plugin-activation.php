...
function uicore_ajax_plugins(){
    if ( ! check_ajax_referer( 'uicore_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
        wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'lumi' ) ) );
    }
    $json = array();
    ...
    foreach ( $plugins as $plugin ) {
        if ( $_POST['slug'] == $plugin['slug'] ) {
            $json = array(
                ...
                'action2'       => - 1,
                'message'       => esc_html__( 'Installing...', 'lumi' ),
            );
            break;
        }
    }
...
if (!function_exists('uicore_plugins_activation')) :
    ...
    function uicore_plugins_activation()
    {
        ...
        $config = array(
            'id'           => 'lumi',            // Unique ID for hashing notices for multiple instances of TGMPA.
            ...
            'strings'      => array(
                'page_title'                      => esc_attr__( 'Install Recommended Plugins', 'lumi' ),
                'menu_title'                      => esc_attr__( 'Install Plugins', 'lumi' )
            )
        );
...
function uicore_plugin_notice(){
    ...
    $elementor_plugins = array(
        ...
        'title' => array(
            __('UiCore Framework', 'lumi'),
            __('Elementor', 'lumi'),
            __('UiCore Animate', 'lumi'),
            __('UiCore Elements', 'lumi'),
        ),
        'class_check' => array(
            '\UiCore\Core',
            '\Elementor\Plugin',
    ...
    foreach ( $not_installed_or_activated_plugins_id as $key => $value ) {
        ...
        $links_attrs[$key] = array(
            ...
            'data-activating-label' => sprintf( esc_attr__( 'Activating %s', 'lumi' ), $progress_text_and_title ),
            'data-installing-label' => sprintf( esc_attr__( 'Installing %s', 'lumi' ), $progress_text_and_title ),
            'data-activate-label'   => sprintf( esc_attr__( 'Activate %s'  , 'lumi' ), $progress_text_and_title ),
            'data-install-label'    => sprintf( esc_attr__( 'Install %s'   , 'lumi' ), $progress_text_and_title ),
        ...
        if( ! isset( $installed_plugins[ $plugins['base_name'][$value] ] ) ){
            ...
            $links_attrs[$key]['button_label'] = sprintf( esc_html__( 'Install %s', 'lumi' ), $progress_text_and_title );
            ...
            $links_attrs[$key]['button_label'] = sprintf( esc_html__( 'Activate %s', 'lumi' ), $progress_text_and_title );
        }
    }
?>
<script>
...
.uicore-btn-active {
    ...
    <div class="uicore-msg" style="background-image:url(<?php echo esc_url(get_template_directory_uri()); ?>/assets/img/demo-import-bg.jpg)">
        <h3 class="uicore-title-default"><?php printf( __( 'Thanks for choosing %s', 'lumi' ), UICORE_THEME_NAME ); ?></h3>
        <?php if (function_exists('_uicore_pro')) : ?>
            <?php if (get_option('uicore_is_gutenberg', null) !== null) : ?>
                <p class="uicore-description-default"><?php echo __( 'Please install and activate the required plugins to gain access to all the theme functionalities and features.', 'lumi' ); ?></p>
            <?php else : ?>
                <p class="uicore-description-default"><?php echo __( 'Please select the main builder you want to use', 'lumi' ); ?></p>
            <?php endif; ?>
        ...
        <p class="submit">
            ...
            <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'uicore-fw-notice', 'install' ), 'uicore-fw-notice', '_notice_nonce' ) ); ?>" class="notice-dismiss uicore-close"><span class="screen-reader-text"><?php _e( 'Skip', 'lumi' ); ?></span></a>
        </p>
        <?php endif; ?>
    </div>
<?php
...
function uicore_hide_notice() {
    ...
    if ( isset( $_GET['uicore-fw-notice'] ) && isset( $_GET['_notice_nonce'] ) ) {
        if ( ! wp_verify_nonce( $_GET['_notice_nonce'], 'uicore-fw-notice' ) ) {
            wp_die( __( 'Authorization failed. Please refresh the page and try again.', 'lumi' ) );
        }
